...
concat = depthConcatenationLayer(3, 'Name', 'concatLayer')
...
skipConv = convolution2dLayer(1,64, 'Name', 'skipConv')
maxP4 = maxPooling2dLayer(4, 'Stride', 4, 'Name', 'pool4')
% Inception model
incepConv1 = convolution2dLayer(1,32,'Name', 'incepConv1')
incepConv2 = convolution2dLayer(5,64, 'Padding', 2, 'Name', 'incepConv2')
maxP5 = maxPooling2dLayer(4, 'Stride', 4, 'Name', 'pool5')

lgraph = layerGraph(layers)
lgraph = addLayers(lgraph, skipConv)
lgraph = addLayers(lgraph, incepConv1)
lgraph = addLayers(lgraph, incepConv2)
lgraph = addLayers(lgraph, maxP4)
lgraph = addLayers(lgraph, maxP5)
lgraph = connectLayers(lgraph,'relu1','skipConv')
lgraph = connectLayers(lgraph,'skipConv','pool4')
lgraph = connectLayers(lgraph,'relu1','incepConv1')
lgraph = connectLayers(lgraph,'incepConv1','incepConv2')
lgraph = connectLayers(lgraph, 'pool4','concatLayer/in2')
lgraph = connectLayers(lgraph, 'incepConv2','pool5')
lgraph = connectLayers(lgraph, 'pool5','concatLayer/in3')
...
global best_Accuracy bestNet bestinfor
best_Accuracy = 0
bestNet = []
...
for j = 1: n
    ...
    modelName = sprintf('%dvali_%depoch_%dbatch_%.3flearnRate.mat', j, epoch_set, minibatch)
    ...
    newResult = table(j,epoch_set, minibatch, currentmeanPixel_Accuracy, {modelName},...
        'VariableNames', {'N_vali','maxEpoch', 'miniBatchSize', 'valiAccuarcy', 'modelName'})
...
figure
confusionChart = confusionchart(metrics.ConfusionMatrix.Variables, ...
    classNames,'Normalization','row-normalized');
confusionChart.Title = 'Normalized Confusion Matrix(%)';
hFig = gcf
saveas(hFig, 'confusion_chart.png')
...
xlabel('IoU Values');
ylabel('Frequency');
title('Mean IoU Histogram');
hFig = gcf
saveas(hFig, 'IoU_chart.png')
